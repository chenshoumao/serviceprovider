...
public class ChangeChallengeResponseServlet extends HttpServlet {
	...
	private void updateChallengesResponse(HttpServletRequest request,
		...
		Map carsToDisplay = new HashMap<String, String>();
		try {
			ChallengeResponseManager crm = new ChallengeResponseManager(
					platform, subject);
			session.setAttribute(CR_MANAGER, crm);
			
//			crm.setChallengesAndResponses(cAndr);
//			ChallengesAndResponses chall  
			//ChallengesAndResponses chall  = new ChallengesAndResponses();
			...
			if ((carsToDisplay != null) && (!carsToDisplay.isEmpty())) {
				...
				crm = (ChallengeResponseManager) session
			...
			//oToPage(MAIN, e.getMessage(), session, request, response);
		}
