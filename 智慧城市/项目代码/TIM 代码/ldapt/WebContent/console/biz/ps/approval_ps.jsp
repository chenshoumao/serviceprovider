...
$.getJSON(url11, function(res) {
    document.getElementById("id_project").value=res[0].id_project; 
    document.getElementById("11ppo").value=res[0].po_no;	
    document.getElementById("sproj_no").value=res[0].proj_no;
    document.getElementById("smake").value=res[0].make;
    document.getElementById("spdesc").value=res[0].proj_desc;	
    document.getElementById("smanu").value=res[0].manufacturer;
    document.getElementById("sia").value=res[0].ia_status;
 });

var  url = '<%=basePath%>framework/ps/approval.action';
function saveBean(){
    $('#fm').form('submit',{
        url: url,
        onSubmit: function(){
...
<body>
	...
	<div >
		<a href="javascript:void(0)" class="easyui-linkbutton c6"	iconCls="icon-ok" onclick="saveBean()" style="width: 90px">Approval</a>
	</div>
